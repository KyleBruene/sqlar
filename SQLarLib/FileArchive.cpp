...
ResultT<std::unique_ptr<FileArchive>> 
			   FileArchive::Open(std::string filepath, FileArchive::Mode mode) // ALWAYS OpenExistingOnly
							{
								using MyResult = ResultT<std::unique_ptr<FileArchive>>;

								auto ret = std::unique_ptr<FileArchive>(new FileArchive());// std::make_unique<FileArchive>();

								FileArchive * fa = ret.get();
								...
								switch (mode)
								{
									...
									default:
										return ResultT<std::unique_ptr<FileArchive>>(false, "Unknown/Unimplemented archive FileArchive::Mode::______");
								}
								...
								return MyResult(true, std::move(ret));
							}
...
Result         FileArchive::Add(std::string dstArchivePath, std::string srcOSPath, bool noCompress, bool verbose/* = true */)
							{
								...
								ResultT<char *> rRes = read_reg_file(zFilenameSrcOSPath, &szOrig, &szCompr, noCompress);
...
std::vector<std::string> 
			   FileArchive::Filenames(std::string searchSubstr)
							{
								std::vector<std::string> ret;
	           ...
	           FileArchive::Get(std::string archivePath)
							{
								...
								using MyResult = ResultT<std::tuple<char *, size_t>>;
...
Result         FileArchive::Put(std::string archivePath, char const * ptr, int numBytes, bool noCompress/* = false*/, bool verbose/* = true*/)
							{
								...
								ResultT<char *> rRes = compress_arr(numBytes, ptr, &szZ, noCompress);// read_reg_file(zFilenameSrcOSPath, &szOrig, &szCompr, noCompress);
